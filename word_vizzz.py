...
def solar_plot(data,name,radius=10,rotations=20,templates='plotly_dark',top_higher=False,n_ratio=1.5,add_image=False,image_path=None):
    ...
    fig=px.scatter(df,x='x',y='y',text='Words',size='Count',size_max=40,color='Count',
               color_continuous_scale='solar',labels={'x':'','y':''},template=templates,animation_frame='Time')
    fig.update_xaxes(range=[-radius-6,radius+6],showgrid=False,zeroline=False,showticklabels=False)
    fig.update_yaxes(range=[-radius**2-18,radius**2+18],showgrid=False,zeroline=False,showticklabels=False)
    if add_image :
        try:
...
def make_wordcloud(data,name,title='text',mask=None,image_path=None):
    ...
    tex=join_n_clear(data,name)
    wordcloud = WordCloud( background_color="white",color_func=random_color_func, max_words=1000, mask=mask,
                         height=1500,width=1500).generate(tex)
    if image_path :
...
def make_random_color(number_of_colors=1,typ=1):
    if typ==1:
        ...
        color = ["#"+''.join([random.choice('0123456789ABCDEF') for j in range(6)])+',0.8'
                     for i in range(number_of_colors)]
        if number_of_colors==1:
            return color[0]
        else:
...
def parellal_word_chart(data_2,name,word=None,number_rows=2,number_cols=4,preprocess_text=False,color_scale=px.colors.sequential.Sunset):
    '''
    Data: name of the dataframe
    ...
    fig = px.parallel_categories(dataframe, dimensions=[i for i in range(number_cols)],
                                 color_continuous_scale=color_scale,color='color')
    fig.show()
            
def sunburst_plot(data_2,name,word=None,number_rows=2,number_cols=4,preprocess_text=False,color_scale=px.colors.sequential.Sunset):
    '''
    Data: name of the dataframe
    name : name of the text column
    word : if there is a specific word you wanna make the plot for 
    ...
    if len(dataframe.columns)>2 :
        ...
        npp=1
    dataframe['color']=pd.factorize(dataframe[npp])[0]
    fig = px.sunburst(dataframe, path=[i for i in range(number_cols)],
                                 color_continuous_scale=color_scale,color='color')
    fig.update_layout(annotations = [dict(text = 'Most probs next word distribution', 
                                          x = 0.5, y = 1.15, font_size = 24, showarrow = True, 
                                          font_family = 'Arial Black',
                                          font_color = 'black')])

    fig.show()
def Link_Plot(data_2,name,word="link",x_factor=2,n_iterations=10,preprocess_text=False,lemmatize_word=True):
    '''
    ...
    try:
        ...
        df=pd.DataFrame(columns=['First word','Middle word','Last word'],data=words)
        counts=0
        ...
        while n_iterations!=0 and counts<len(existing_word): 
            ...
            for i in range(len(first_word)):
                dn=df[(df['First word']==first_word[i]) & (df['Last word']==last_word[i]) & (~df['Middle word'].isin(existing_word))]
        ...
        link = dict(
          source = anr[:,0], # indices correspond to labels, eg A1, A2, A1, B1, ...
          target = anr[:,1],
          value = anr[:,2]
        ))])

        fig.update_layout(title_text="Closest Relatable Word"   , font=dict(size = 20, color = 'black'),
        plot_bgcolor='#252525',
